...
% who are mine
app.PCNNeuronsTextArea_PCN.Value = num2str(app.PCNs{stimNum});

%Who is total
all_neur = [];
for i = setdiff([1:size(app.params.UDF,2)],stimNum)
    all_neur = [all_neur app.PCNs{i}]; 
end

%who is unique
unique_neurons = setdiff(app.PCNs{stimNum},all_neur);
...
%Who is promiscuous
prom_neurons = setdiff(app.PCNs{stimNum},unique_neurons);
