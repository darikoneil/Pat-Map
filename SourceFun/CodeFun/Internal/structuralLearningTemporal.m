function [params] = structuralLearningTemporal(params)
...
removeRecurrent = restrictions(params.learned_structures{1});

for i = 1:length(params.learned_structures)
    params.learned_structures{i} = params.learned_structures{i}.*removeRecurrent;
    tmp = params.learned_structures{i};
    if sum(tmp)<2
        tmpRem = [tmpRem i];
    end
end
