classdef PatMap < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        % Size changed function: IOPanel
        function IOPanelSizeChanged(app, event)
            app.IOPanel.Visible='off';
            sz = get(groot, 'ScreenSize');
            app.IOPanel.Position(3) = sz(3);
            app.IOPanel.Visible='on';
        end

        % Callback function
        function ParamsPanelSizeChanged(app, event)
            position = app.ParamsPanel.Position;
            disp('69')
            sz = get(groot, 'ScreenSize');
            app.ParamsPanel.Visible='off';
            val = app.UIFigure.Position(4)-250-700-22-23;
            app.ParamsPanel.Position = [app.UIFigure.Position(3)-969 val 910 700];
            app.ParamsPanel.Visible='on';
            b = 99
        end
        ...
        function UIFigureSizeChanged(app, event)
            ...
            sz = get(groot, 'ScreenSize');
            app.UIFigure.Position(3:4) = sz(3:4);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.IOPanel.SizeChangedFcn = createCallbackFcn(app, @IOPanelSizeChanged, true);
