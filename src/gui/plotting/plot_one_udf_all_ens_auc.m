...
p(1) = plot(app.ensemble_one_udf_all_ens_auc, perf.fpr{udf, udf},perf.tpr{udf,udf},'LineWidth',2,'Color',[0.47 0.25 0.8]);
p(2) = fill(app.ensemble_one_udf_all_ens_auc,perf.fpr{udf,udf},perf.tpr{udf,udf},[0.47 0.25 0.8],'EdgeAlpha',0,'FaceAlpha',0.25);

for i = 1:length(all_udf)
      p(i+2) = plot(app.ensemble_one_udf_all_ens_auc,perf.fpr{udf, all_udf(i)},perf.tpr{udf, all_udf(i)},'LineWidth',2,'Color',[newcolors(z,:)]);
      z=z+1;
      %fill(app.ensemble_one_udf_all_ens_auc,perf.fpr{udf,i},perf.tpr{udf,i},[newcolors(i,:)],'EdgeAlpha',0,'FaceAlpha',0.25);
end
p(num_udf +2) = plot(app.ensemble_one_udf_all_ens_auc,[0 1],[0 1],'LineStyle','--','Color','k','LineWidth',1);
p(num_udf + 3) = fill(app.ensemble_one_udf_all_ens_auc,[0 1 1],[0 1 0], [0.25 0.8 0.54],'EdgeAlpha',0,'FaceAlpha',0.25);
...
idx = [1 3:(length(p)-1)];
legend(app.ensemble_one_udf_all_ens_auc, p(idx), legend_string, 'Location', 'southeast');
