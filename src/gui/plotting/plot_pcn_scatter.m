...
% need
PCNs = app.pattern_completion_nodes{udf};
...
scatter(app.pattern_completion_scatter, NodeScores, auc(:,udf), 50, 'k', 'linewidth', 1, 'MarkerFaceAlpha',0);

% plot the filled points
hold(app.pattern_completion_scatter,'on');
scatter(app.pattern_completion_scatter, NodeScores(PCNs),auc(PCNs,udf), 50, 'MarkerFaceColor', newcolors(3,:), 'linewidth',1, 'MarkerEdgeColor','k');
hold(app.pattern_completion_scatter,'off');

% ensembles
hold(app.pattern_completion_scatter,'on');
scatter(app.pattern_completion_scatter, NodeScores(app.ensemble_nodes{udf}),auc(app.ensemble_nodes{udf},udf), 50, 'MarkerFaceColor', newcolors(4,:), 'linewidth',1, 'MarkerEdgeColor','k');
hold(app.pattern_completion_scatter,'off');

% plot the intervals
hold(app.pattern_completion_scatter,'on');
plot(app.pattern_completion_scatter,[nsmi nsma],AucThr(2,udf)*[1 1],'k--','LineWidth',1);
...
if app.DeviationsEditField.Value>1
    SingleDev = AucThr(3,udf)-AucThr(2,udf); 
    AucLowThr = AucThr(2,udf)-((app.DeviationsEditField.Value)*SingleDev);
    AucHighThr = AucThr(2,udf)+((app.DeviationsEditField.Value)*SingleDev);
...
if strcmp(app.NodeThresholdDropDown.Value,'Ensemble')
    plot(app.pattern_completion_scatter, NodeThr(2,udf)*[1 1],[aucmi aucma],'k--','LineWidth',1);
    plot(app.pattern_completion_scatter, NodeThr(1,udf)*[1 1],[aucmi aucma],'k--','LineWidth',1);
    plot(app.pattern_completion_scatter, NodeThr(3,udf)*[1 1],[aucmi aucma],'k--','LineWidth',1);
    hold(app.pattern_completion_scatter,'off');
else
...
hold(app.pattern_completion_scatter,'on');
if strcmp(app.NodeThresholdDropDown.Value,'Ensemble')
    X = [NodeThr(1,udf) NodeThr(3,udf) NodeThr(3,udf) NodeThr(1,udf)];
else
    X = [NodeThr(1) NodeThr(3) NodeThr(3) NodeThr(1)];
end

Y = [AucThr(1,udf) AucThr(1,udf) AucThr(3,udf) AucThr(3,udf)];
