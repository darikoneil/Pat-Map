...
i = 1;
fill(app.ensemble_coordinates,ROI_contours.xpix{i}(ROI_contours.boundaryOutlines{i}),ROI_contours.ypix{i}(ROI_contours.boundaryOutlines{i}),'w','FaceAlpha', 0.25,'EdgeAlpha',1, 'LineWidth',1.5);
...
for i = 1:length(ensemble_rois.xpix)
    fill(app.ensemble_coordinates,ensemble_rois.xpix{i}(ensemble_rois.boundaryOutlines{i}),ensemble_rois.ypix{i}(ensemble_rois.boundaryOutlines{i}),ensemble_colors(i,:),'FaceAlpha',0.75, 'EdgeAlpha',1, 'LineWidth',1.5);
end

%xlim(app.ensemble_coordinates,[xrange]);
%ylim(app.ensemble_coordinates,[yrange]);
