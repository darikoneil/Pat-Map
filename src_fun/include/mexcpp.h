...
namespace mexcpp {
  ...
  struct Mat : public BaseMat {
    ...
    T min() const { return *std::min_element(re, re + length); }
    T max() const { return *std::max_element(re, re + length); }
    ...
    typedef Eigen::Map<EigenArray> EigenArrayMap;
    ...
    EigenMatrixMap asEigenMatrixMap() {
      return EigenMatrixMap(re, M, N);
    }

    EigenArray asEigenArray() {
      EigenArray a(length);
      std::copy(re, re + length, a.data());
      return a;
    }

    EigenArrayMap asEigenArrayMap() {
      return EigenArrayMap(re, M, N);
    }
#endif
